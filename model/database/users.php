...
class users {
	 ...
	 public $users=array();
                ...
                public function role($id, $first_name, $last_name, $matricule, $phone_number, $email, $type_agent, $created_at, $updated_at, $photo, $role, $status, $domaine, $region, $password_, $profil)
                    {
                        $this->id = $id;
...
$this->users =$data; 
 return $this;
                    ...
                    */ 
                    ...
                    {
                        if ($domaine != null && is_array($this->users) && count($this->users)!=0) {
                            $table_name = strtolower(get_class($this));
...
$this->users =$data; 
 return $this;
                    ...
                    */ 
                    ...
                    {
                        if ($region != null && is_array($this->users) && count($this->users)!=0) {
                            $table_name = strtolower(get_class($this));
...
$this->users =$data; 
 return $this;
                    ...
                    */ 
                    ...
                    {
                        if ($profil != null && is_array($this->users) && count($this->users)!=0) {
                            $table_name = strtolower(get_class($this));
                            $query = "SELECT * FROM $table_name WHERE profil = ?";
                            $req = Manager::bdd()->prepare($query);
                            $req->execute([$profil]);
                            $data = "";
                            if ($data = $req->fetchAll(PDO::FETCH_ASSOC)) {
